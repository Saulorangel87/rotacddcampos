...
export default function AjustesRotasPanel({
  distritoOrigem,
  onFechar,
  onConcluido,
}) {
  ...
  const ruasFiltradas = ruas.filter((r) =>
    r.nome_rua.toLowerCase().includes(busca.toLowerCase()),
  );
  const totalPaginas = Math.max(
    1,
    Math.ceil(ruasFiltradas.length / RUAS_POR_PAGINA),
  );
        ...
        <ol className={styles.passos}>
          {PASSOS.map((nome, i) => (
            <li
              key={nome}
              data-estado={
                i === passo ? "ativo" : i < passo ? "concluido" : "pendente"
              }
            >
        ...
        {passo === 0 && (
          <div className={styles.conteudo}>
            <p className={styles.aviso}>
              Selecione as ruas que serão movidas para outro distrito.
            </p>
            <input
              type="search"
        ...
        {passo === 2 && resultado && (
          <div className={styles.conteudo}>
            <div className={styles.sucesso}>
              ...
              <p>
                {resultado.quantidade} rua(s) foram movidas do distrito{" "}
                {resultado.de || "—"} para o distrito {resultado.para}.
              </p>
            </div>
            <footer className={styles.rodapePasso}>
              <button
